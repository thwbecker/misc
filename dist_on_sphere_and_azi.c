...
compute distance on sphere and azimuth in degrees for a station at
lon0, lat0 as read in from the command line, and a series of other
(source) points as read from stdin, all in degrees
...
$Id: dist_on_sphere_and_azi.c,v 1.4 2005/04/12 19:07:07 becker Exp twb $
...
int main(int argc, char **argv)
{
  ...
  /* stattion coordinates in degrees */
  sscanf(argv[1],"%lf",&lon0d);
  sscanf(argv[2],"%lf",&lat0d);
  if((lat0d < -90)||(lat0d>90)||(lon0d<-180)||(lon0d>360)){
    fprintf(stderr,"%s: out of bounds: lon0: %g lat0: %g\n",
	    argv[0],lon0d,lat0d);
    exit(-1);
  }else
    if(verbose)
      fprintf(stderr,"%s: using %g, %g as origin (station) coordinates\n",argv[0],lon0d,lat0d);
  if(argc>3)
    sscanf(argv[3],"%i",&azimode);
  if(verbose){
    if(azimode)
  ...
  /* 
     convert to radians 
  */
  lon0 = lon0d / PIF;lat0 = lat0d / PIF;
  colat0d = 90 - lat0d;
  /* 
     sin/cos 
  */
  slat[0]=sin(lat0);clat[0]=cos(lat0);
  while(fscanf(stdin,"%lf %lf",&lond,&latd)==2){
    if(mode == 0){
      /* convert to radian */
