...
int main(int argc, char *argv[] )
{
  ...
  nrp--;
  if(verbose)fprintf(stderr,"%s: read %i contour points\n",argv[0],nrp);
  if(!nrp){
    fprintf(stderr,"%s: Exiting, no data.\n",argv[0]);exit(-1);}

  fit(x-1,y-1,nrp,sigy-1,1,&a, &b,&siga,&sigb,&chi2,&q);
