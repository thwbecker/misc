...
/*
  ...
  $Id: splinesc.c,v 1.1 2001/07/14 22:27:21 becker Exp $
  
*/

#define MAX_SPLINE_N 50
COMP_PRECISION spline(COMP_PRECISION x1,COMP_PRECISION x2,
		      ...
		      COMP_PRECISION x)
{
  ...
  // check range 
  if ((x-x1)*(x-x2) > 0.0){
    fprintf(stderr,"spline: x (%g) not in range (%g - %g)\n",
	    x,x1,x2);
    exit(-1);
  }
  if(!init){
    if(m > MAX_SPLINE_N){
      fprintf(stderr,"spline: works only for order <= %i (m: %i)\n",
	      MAX_SPLINE_N,m);
      exit(-1);
    }
