...
point read from stdin is within <= dist, where dist is in km

output is only if close, then 
...
{
  float lon0,lat0,dist_km,lon,lat,depth,radius,z;
  int n,i;
  depth = 0;
  if(argc < 4){
    fprintf(stderr,"usage: %s lon0 lat0 dist(radius)_km [depth_km, %g]\nread lon lat z and passes, if close\n",
	    argv[0],depth);
    exit(-1);
  }
  ...
  n=i=0;
  while(fscanf(stdin,"%f %f %f",&lon,&lat,&z)==3){
  ...
  fprintf(stderr,"%s: performed %i tests, passed %i \n",
	  argv[0],n,i);
  return 0;
}
		  ...
		  float lon2,float lat2, float radius)
{
  ...
  tmp3=sqrt(tmp1+tmp2);
  return 2.0*asin(tmp3)*R_E*radius;
}
