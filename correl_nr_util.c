...
#include "correl_nr.h"
/* 
   x and y are n dim on input, nn on output
 */
void compute_correl(double **x, double **y, double **corr, int n, int *nn)
{
  ...
  *x=(double *)realloc(*x,sizeof(double)*(*nn));
  *y=(double *)realloc(*y,sizeof(double)*(*nn));
  *corr=(double *)realloc(*corr,sizeof(double)*(*nn)*2);
  if(!x || !y || !corr)ME;
  for(i=n;i<(*nn);i++){// zero pad
    *(*x+i) = *(*y+i) = 0.0;
  }
  // calculate correlation, numrec style call
  correl((*x-1),(*y-1),(*nn),(*corr-1));
